...
describe('pillarDeleteCommandHandler', () => {
	it('should call methods in established order: validate, createEvent, dispatch, persist, publish', () => {
		let callCounter = 0;

		const pillarDeleteCommandHandler = proxyquire('./index', {
			'./pillar_delete.cmd.validator': () => {
	...
	it('should throw an error when a validation fails', () => {
		const pillarDeleteCommandHandler = proxyquire('./index', {
			...
			'./pillar_deleted.event.creator': () => null,
