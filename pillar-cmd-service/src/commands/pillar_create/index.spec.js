...
describe('pillarCreateCommandHandler', () => {
	it('should call methods in established order: validate, createEvent, dispatch, persist, publish', () => {
		let callCounter = 0;

		const pillarCreateCommandHandler = proxyquire('./index', {
			'./pillar_create.cmd.validator': () => {
	...
	it('should throw an error when a validation fails', () => {
		const pillarCreateCommandHandler = proxyquire('./index', {
			...
			'./pillar_created.event.creator': () => null,
