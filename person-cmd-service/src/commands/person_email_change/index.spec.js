...
describe('personEmailChangeCommandHandler', () => {
	it('should call methods in established order: validate, createEvent, dispatch, persist, publish', () => {
		let callCounter = 0;

		const personEmailChangeCommandHandler = proxyquire('./index', {
			'./person_email_change.cmd.validator': () => {
			...
			'./person_email_changed.event.creator': () => {
		...
		return personEmailChangeCommandHandler({})
			.catch(err => {
	...
	it('should throw an error when a validation fails', () => {
		const personEmailChangeCommandHandler = proxyquire('./index', {
			...
			'./person_email_changed.event.creator': () => null,
