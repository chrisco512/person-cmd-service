...
describe('personLastNameChangeCommandHandler', () => {
	it('should call methods in established order: validate, createEvent, dispatch, persist, publish', () => {
		let callCounter = 0;

		const personLastNameChangeCommandHandler = proxyquire('./index', {
			'./person_last_name_change.cmd.validator': () => {
			...
			'./person_last_name_changed.event.creator': () => {
	...
	it('should throw an error when a validation fails', () => {
		const personLastNameChangeCommandHandler = proxyquire('./index', {
			...
			'./person_last_name_changed.event.creator': () => null,
