...
describe('pointIncrementCommandHandler', () => {
	it('should call methods in established order: validate, createEvent, dispatch, persist, publish', () => {
		let callCounter = 0;

		const pointIncrementCommandHandler = proxyquire('./index', {
			'./point_increment.cmd.validator': () => {
			...
			'./point_incremented.event.creator': () => {
		...
		return pointIncrementCommandHandler({})
			.catch(err => {
	...
	it('should throw an error when a validation fails', () => {
		const pointIncrementCommandHandler = proxyquire('./index', {
			...
			'./point_incremented.event.creator': () => null,
